export function createTrustedSurfaceRuntime({
  ...
  function identityLayout(surface) {
    const base = surface.baseBounds; const viewport = getRuntimeViewport(surface.context);
    const width = Math.max(190, Math.min(base.width, Math.max(190, viewport.width - 8)));
    const height = Math.max(50, Math.min(base.height, 110));
    const x = Math.max(4, Math.min(base.x, Math.max(4, viewport.width - width - 4)));
    const y = Math.max(4, Math.min(base.y, Math.max(4, viewport.height - height - 4)));
    return { x: viewport.x + x, y: viewport.y + y, width, height };
  }
  ...
  function applyIdentityLayout(surface) {
    ...
    surface.view.setBounds(identityLayout(surface)); raiseExpandedAuth();
  }
  ...
  function createIdentity(context, anchor) {
    ...
    const surface = { key: keyFor(context, anchor.id), anchorId: anchor.id, context, view, baseBounds: { x: anchor.x, y: anchor.y, width: anchor.width, height: anchor.height }, destroyed: false };
  ...
  ipcMain.on('wurst:identity:anchors', (event, rawAnchors) => {
    ...
    for (const raw of anchors) {
      ...
      const x = Math.round(Number(raw?.x)); const y = Math.round(Number(raw?.y));
      if (!id || !raw?.visible || !Number.isFinite(width) || !Number.isFinite(height) || width < 190 || height < 50 || ![x, y].every(Number.isFinite)) continue;
      const anchor = { id, width, height, x, y }; const key = keyFor(context, id); seen.add(key);
      const existing = identitySurfaces.get(key);
      if (existing) { existing.baseBounds = { x, y, width, height }; applyIdentityLayout(existing); } else createIdentity(context, anchor);
    }
