...
Normal pushes to `main` do not deploy Pages. `.github/workflows/release.yml` publishes the Windows, macOS and Web assets first and then calls the Pages workflow with the exact release commit. A manual Pages run also requires an explicit tag or commit, so recovery deployments cannot accidentally publish whatever happens to be newest on `main`.
