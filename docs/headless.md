...
Wurster Lab 0.32.5 includes a browserless harness for development, CI and automation:
...
The 0.32.5 CLI harness uses a disposable Node worker and restricted JavaScript context. It is useful for developer-controlled Wursts and CI, but it is **not** the final hostile-third-party-code sandbox. A production headless Wurster still needs a real untrusted-code boundary with CPU, memory and capability budgets.
