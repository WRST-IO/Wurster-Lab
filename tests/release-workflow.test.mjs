...
const electronSmokePrepare = await fs.readFile(path.join(root, 'tools', 'prepare-electron-smoke.mjs'), 'utf8');
const electronSmoke = await fs.readFile(path.join(root, 'tools', 'electron-piglet-route-smoke.mjs'), 'utf8');
...
assert.match(release, /actions\/cache\/restore@v5/);
assert.match(release, /actions\/cache\/save@v5/);
assert.match(release, /electron_config_cache/);
assert.match(release, /node tools\/prepare-electron-smoke\.mjs/);
assert.match(release, /WURSTER_ELECTRON_PREPARE_TIMEOUT_MS: '240000'/);
assert.match(release, /WURSTER_ELECTRON_SMOKE_TIMEOUT_MS: '30000'/);
assert.match(release, /timeout-minutes: 2/);
assert.equal((release.match(/Prepare Electron binary for smoke/g) || []).length, 3);
assert.equal((release.match(/Verify real Electron Desktop Piglet route/g) || []).length, 3);
assert.equal((release.match(/actions\/cache\/restore@v5/g) || []).length, 3);
assert.equal((release.match(/actions\/cache\/save@v5/g) || []).length, 3);
assert.match(electronSmokePrepare, /WURSTER_ELECTRON_PREPARE_TIMEOUT_MS/);
assert.match(electronSmokePrepare, /child\.kill\('SIGTERM'\)/);
assert.match(electronSmokePrepare, /Electron smoke binary preparation timed out/);
assert.match(electronSmoke, /Promise\.race\(\[run\(\), smokeTimeout\]\)/);
