...
assert.equal(pkg.scripts['test:electron:piglet'], undefined);
await assert.rejects(fs.stat(path.join(root, 'tools', 'prepare-electron-smoke.mjs')), /ENOENT/);
await assert.rejects(fs.stat(path.join(root, 'tools', 'electron-piglet-route-smoke.mjs')), /ENOENT/);
assert.doesNotMatch(release, /test:electron:piglet/);
assert.doesNotMatch(release, /prepare-electron-smoke/);
assert.doesNotMatch(release, /electron-smoke/);
assert.doesNotMatch(release, /Electron Desktop Piglet route/);
assert.doesNotMatch(release, /WURSTER_ELECTRON_(?:PREPARE|SMOKE)_TIMEOUT_MS/);
assert.doesNotMatch(release, /electron_config_cache/);
assert.doesNotMatch(release, /actions\/cache\/(?:restore|save)@v5/);
