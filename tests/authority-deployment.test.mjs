...
const workerSource = await fs.readFile(
  path.join(root, 'authority', 'wrst.io', 'worker', 'src', 'index.js'),
  'utf8'
);
...
assert.equal(
  await fs.stat(removedEmailConfig).then(() => true, () => false),
  false,
  'There must be one current Worker config, not an email variant'
);
assert.equal(
  Object.hasOwn(workerConfig, 'send_email'),
  false,
  'Cloudflare Email Sending binding must not be part of the Authority'
);
assert.equal(
  Object.hasOwn(workerConfig, 'migrations'),
  false,
  'Pre-1.0 Worker config must not carry legacy DO migrations'
);
...
assert.deepEqual(
  workerConfig.secrets.required.sort(),
  [
...
assert.equal(
  workerSource.includes("url.pathname === '/.well-known/wurst-authority'"),
  false
);
...
const relayVectorValue =
  '1786615200\nnonce_abcdefghijklmnopqrstuvwxyz\n{"format":"wrst/mail-relay-request-1"}';

const workerHmac = await workerTest.relayHmacHex(
  relayVectorSecret,
  relayVectorValue
);

const phpHmac = spawnSync(
  'php',
  [
    '-r',
    `echo hash_hmac('sha256', ${JSON.stringify(relayVectorValue)}, ${JSON.stringify(relayVectorSecret)});`
  ],
  { encoding: 'utf8' }
);
...
assert.equal(
  phpHmac.stdout,
  workerHmac,
  'Worker and PHP relay must sign the exact same request bytes'
);
...
const smtpServer = net.createServer((socket) => {
  ...
  socket.on('error', (error) => {
    // A client may reset the TCP connection while cleaning up after a complete
    // SMTP conversation. Once QUIT was received, that is no longer a protocol
    // failure. Any socket error before QUIT remains a real test failure.
    if (error?.code === 'ECONNRESET' && protocolFinished) return;
    smtpSocketErrors.push(error);
  });
  ...
  socket.on('data', (chunk) => {
    ...
    while (true) {
      ...
      } else if (line === Buffer.from('relay-user').toString('base64')) {
        socket.write('334 UGFzc3dvcmQ6\r\n');
      } else if (line === Buffer.from('relay-pass').toString('base64')) {
...
$message = wrst_build_message(
    [
        'to' => 'erna@example.test',
        'code' => '381492',
        'publisherFingerprint' => str_repeat('a', 64),
...
const smtpChild = spawn('php', [smtpPhp], {
  stdio: ['ignore', 'pipe', 'pipe']
});

let smtpErr = '';

smtpChild.stderr.on('data', (chunk) => {
  smtpErr += chunk;
});

const smtpExit = await new Promise((resolve, reject) => {
  smtpChild.once('error', reject);
  smtpChild.once('close', resolve);
});
...
console.log(
  '✓ Authority deployment is split into static GitHub Pages discovery, cryptographic Cloudflare issuance and a configurable HMAC-authenticated PHP mail relay'
);