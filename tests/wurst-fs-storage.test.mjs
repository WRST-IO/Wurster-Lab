...
const { store: crud, bytes: crudBytes } = await createMemoryWurstFs2Store(Buffer.from('CRUD-BASE'));
...
const big = crud.beginWrite('/data/files/big-video.bin');
await crud.writeChunk(big, Buffer.alloc(MiB, 0x41));
const small = crud.beginWrite('/data/files/tiny-video.bin');
await crud.writeChunk(small, Buffer.from('tiny'));
await crud.commitWrite(small);
const note = crud.beginWrite('/data/files/note.txt', { mime: 'text/plain' });
...
assert.equal((await crud.read('/data/files/tiny-video.bin')).data.toString(), 'tiny');
assert.equal((await crud.read('/data/files/note.txt')).data.toString(), 'Currywurst');
assert.equal((await crud.stat('/data/files/big-video.bin')).size, 2 * MiB);
...
const pathOrder = crud.beginWrite('/data/files/tools/export.txt');
await crud.writeChunk(pathOrder, Buffer.from('nested'));
await crud.commitWrite(pathOrder);
const underscorePath = crud.beginWrite('/data/files/tools_export_wurster_lab.py');
await crud.writeChunk(underscorePath, Buffer.from('exporter'));
await crud.commitWrite(underscorePath);
assert.equal((await crud.stat('/data/files/tools_export_wurster_lab.py')).size, 8);
assert.equal((await crud.read('/data/files/tools_export_wurster_lab.py')).data.toString(), 'exporter');

// Concurrent writes to the same logical object conflict instead of silently
// merging bytes from two transactions.
const sameA = crud.beginWrite('/data/files/same.txt');
const sameB = crud.beginWrite('/data/files/same.txt');
await crud.writeChunk(sameA, Buffer.from('A'));
await crud.writeChunk(sameB, Buffer.from('B'));
await crud.commitWrite(sameA);
await assert.rejects(() => crud.commitWrite(sameB), (error) => error.code === 'WURST_FS_CONFLICT');
crud.abortWrite(sameB);
assert.equal((await crud.read('/data/files/same.txt')).data.toString(), 'A');
...
const { store: personal, bytes: personalBytes } = await createMemoryWurstFs2Store(Buffer.from('PERSONAL-BASE'));
...
let ptx = personal.beginWrite('/data/mine/secret.txt', { actor: alice, mime: 'text/plain' });
...
await assert.rejects(() => personal.grant('mine', bob.publicRecord, { read: true }, { actor: alice }), (error) => error.code === 'WURST_FS_NOT_SHAREABLE');
personal.close();
...
const { store: mixed, bytes: mixedBytes } = await createMemoryWurstFs2Store(Buffer.from('MIXED-BASE'));
...
let mtx = mixed.beginWrite('/data/workspace/README.md', { mime: 'text/markdown' });
await mixed.writeChunk(mtx, Buffer.from('# Wurster Lab'));
await mixed.commitWrite(mtx);
assert.throws(() => mixed.unlockRealm('operator', alice), (error) => error.code === 'WURST_FS_UNCLAIMED');
await mixed.claimPersonalRealm('operator', { actor: alice });
assert.equal(mixed.realm('operator').claimed, true);
mtx = mixed.beginWrite('/data/operator/issuer.wurstissuer', { actor: alice });
...
function makeWurst(version, realms) {
  return encodeWurst({
    manifest: {
      ...
      data: { format: 'wurst/data-realms-1', writable: true, realms }, security: { signed: false }
    },
...
try {
  ...
  let disk = await openLocalWurstFsStore(crudFile, reader);
  await disk.initialize({ realms: [{ id: 'files' }] });
  let tx = disk.beginWrite('/data/files/delete-me.bin');
  await disk.writeChunk(tx, Buffer.alloc(2 * MiB, 0x55));
  await disk.commitWrite(tx);
  tx = disk.beginWrite('/data/files/keep.bin');
  await disk.writeChunk(tx, Buffer.alloc(64 * 1024, 0x33));
  await disk.commitWrite(tx);
  await disk.remove('/data/files/delete-me.bin');
  await disk.closeFile();
  await reader.close();

  reader = await openWurstFile(crudFile);
  const usage = await measureWurstFs2Storage(reader.source, reader.wurstFsRoot, {
    baseOffset: reader.baseLength,
    commitOffset: reader.wurstFsCommitOffset
  });
  ...
  assert.equal(await compactReader.fsStat('/data/files/delete-me.bin'), null);
  assert.equal((await compactReader.fsReadRange('/data/files/keep.bin')).data.length, 64 * 1024);
  assert.equal(compactReader.wurstFsRoot.historyMode, 'none');
  ...
  disk = await openLocalWurstFsStore(personalFile, reader);
  await disk.initialize({ actor: alice, rootAdmins: [alice.publicRecord.identityId], realms: [{ id: 'mine', governance: 'personal' }] });
  tx = disk.beginWrite('/data/mine/delete-me.bin', { actor: alice });
  await disk.writeChunk(tx, Buffer.alloc(2 * MiB, 0x77));
  await disk.commitWrite(tx);
  tx = disk.beginWrite('/data/mine/keep.txt', { actor: alice, mime: 'text/plain' });
  await disk.writeChunk(tx, Buffer.from('still secret'));
  await disk.commitWrite(tx);
  await disk.remove('/data/mine/delete-me.bin', { actor: alice });
  ...
  const reopened = await openLocalWurstFsStore(personalCompact, personalReader);
  reopened.unlockRealm('mine', alice);
  assert.equal((await reopened.read('/data/mine/keep.txt')).data.toString(), 'still secret');
  assert.equal(await reopened.stat('/data/mine/delete-me.bin'), null);
...
console.log('✓ WurstFS v2 ordinary storage is the unnamed history-free default without identities or signatures');
