...
import { SEALED_APP_INDEX_PATH, SIGNATURE_PATH, createAuthorityIssuer, createAuthorityRoot, createMemoryWurstFs2Store, createPackageSignature, createPublisherCertificateFromIssuer, createPublisherCertificateRequest, createPublisherKeyBundle, createTrustBundle, decodeWurst, encodeWurst, openWurstFile, sealApplicationFiles } from '../packages/format/src/index.js';
import { MessagePortWurstSource, WurstWebReader, WurstWebFsOverlay, WursterWebSession, verifyPublisherCertificateWeb, verifyTrustBundleWeb } from '../runtime/web/src/wurster-web.mjs';
...
const manifest = {
  ...
  application: { protection: 'public' }, data: { format: 'wurst/data-realms-1', writable: true, realms: [{ id: 'files', label: 'Files' }] }, capabilities: []
};
...
const memoryFs = await createMemoryWurstFs2Store(immutable);
await memoryFs.store.initialize({ realms: [{ id: 'files', label: 'Files' }] });
let seedTx = memoryFs.store.beginWrite('/data/files/flights/demo.json', { mime: 'application/json' });
...
assert.equal((await reader.fsStat('/data/files/flights/demo.json')).size, 21);
assert.equal(new TextDecoder().decode(await reader.fsReadRange('/data/files/flights/demo.json', 10, 9)), 'EDDM-LOWI');

const overlay = new WurstWebFsOverlay(reader, { sessionId: 'web-test' });
const tx = await overlay.beginWrite('/data/files/media/chunky.bin', { mime: 'application/octet-stream' });
...
const crossing = await overlay.read('/data/files/media/chunky.bin', { offset: first.length - 8, length: 24 });
assert.deepEqual([...crossing.subarray(0,8)], Array(8).fill(0x11));
assert.deepEqual([...crossing.subarray(8)], Array(16).fill(0x22));
assert.equal((await overlay.stat('/data/files/media/chunky.bin')).size, first.length + second.length + third.length);
assert.equal((await overlay.capabilities()).streamingWrite, true);

// Renaming a file from the immutable WurstFS base must be metadata-only in the
// overlay; it must not first materialize the complete base file into JS memory.
await overlay.rename('/data/files/flights/demo.json', '/data/files/flights/renamed.json');
assert.equal(await overlay.stat('/data/files/flights/demo.json'), null);
assert.equal(new TextDecoder().decode(await overlay.read('/data/files/flights/renamed.json')), '{"route":"EDDM-LOWI"}');

await overlay.write('/data/files/flights/next.json', '{"route":"KJFK-KBOS"}', { mime: 'application/json' });
assert.equal((await overlay.stat('/data/files/flights/next.json')).mime, 'application/json');
assert.equal(new TextDecoder().decode(await overlay.read('/data/files/flights/next.json')).includes('KJFK'), true);
...
const headResponse = await runtimeSession._serve({ scope: 'data', path: 'files/flights/demo.json', method: 'HEAD', range: 'bytes=-4' });
...
try {
  ...
  try {
    assert.equal(await reopened.fsStat('data/files/flights/demo.json'), null);
    assert.equal((await reopened.fsStat('data/files/flights/renamed.json')).size, 21);
    assert.equal((await reopened.fsStat('data/files/flights/next.json')).size, 21);
    assert.equal((await reopened.fsStat('data/files/media/chunky.bin')).size, first.length + second.length + third.length);
    assert.equal((await reopened.fsReadRange('data/files/flights/next.json', 10, 9)).data.toString(), 'KJFK-KBOS');
  } finally { await reopened.close(); }
} finally { await fs.rm(tmp, { recursive: true, force: true }); }
await overlay.dispose();
console.log('✓ Wurster Web keeps normal relative resources, uses chunk-backed WurstFS writes/ranges and exports a valid standalone snapshot');
